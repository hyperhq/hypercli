...
func (s *DockerSuite) TestApiVolumesList(c *check.C) {
	...
	status, b, err := sockRequest("GET", "/volumes", nil, os.Getenv("REGION"))
...
func (s *DockerSuite) TestApiVolumesCreate(c *check.C) {
	...
	status, b, err := sockRequest("POST", "/volumes/create", config, os.Getenv("REGION"))
...
func (s *DockerSuite) TestApiVolumesRemove(c *check.C) {
	...
	v := volumes.Volumes[0]
	status, _, err = sockRequest("DELETE", "/volumes/"+v.Name, nil, os.Getenv("REGION"))
	c.Assert(err, checker.IsNil)
	...
	status, data, err := sockRequest("DELETE", "/volumes/"+v.Name, nil, os.Getenv("REGION"))
	c.Assert(err, checker.IsNil)
...
func (s *DockerSuite) TestApiVolumesInspect(c *check.C) {
	...
	var vol types.Volume
	status, b, err = sockRequest("GET", "/volumes/"+config.Name, nil, os.Getenv("REGION"))
	c.Assert(err, checker.IsNil)
	c.Assert(status, checker.Equals, http.StatusOK, check.Commentf(string(b)))
