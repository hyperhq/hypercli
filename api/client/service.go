...
func (cli *DockerCli) CmdServiceCreate(args ...string) error {
	...
	var (
		...
		flContainerSize       = cmd.String([]string{"-size"}, "s4", "The type for each instance (e.g. s1, s2, s3, s4, m1, m2, m3, l1, l2, l3)")
		flWorkingDir          = cmd.String([]string{"w", "-workdir"}, "", "Working directory inside the container")
		flSSLCert             = cmd.String([]string{"-ssl-cert"}, "", "SSL cert file for service load balance")
		flServicePort         = cmd.Int([]string{"-service-port"}, 0, "Service publish port for service container")
		flContainerPort       = cmd.Int([]string{"-container-port"}, 0, "Container publish port for container")
		flReplicas            = cmd.Int([]string{"-replicas"}, -1, "Replicas number for container")
		flHealthCheckInterval = cmd.Int([]string{"-health-check-interval"}, 3, "Health check interval for the container")
		flHealthCheckFall     = cmd.Int([]string{"-health-check-fall"}, 3, "Health check fall for container")
		flHealthCheckRise     = cmd.Int([]string{"-health-check-rise"}, 2, "Health check rise for container")
		flSessionAffinity     = cmd.Bool([]string{"-session-affinity"}, false, "Session Affinity")
		flAlgorithm           = cmd.String([]string{"-algorithm"}, types.LBAlgorithmRoundRobin, "Algorithm for load balance")
		flProtocol            = cmd.String([]string{"-protocol"}, types.LBProtocolTCP, "Protocol for load balance")
	)
