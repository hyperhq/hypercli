...
func (cli *DockerCli) CmdComposeDown(args ...string) error {
	...
	options := options.Down{
		...
		RemoveOrphans: *rmorphans,
	}
	if err = project.Down(options, cmd.Args()...); err != nil {
		return err
	}
...
func (cli *DockerCli) CmdComposeUp(args ...string) error {
	...
	err = project.Up(options, cmd.Args()...)
	if err != nil {
		return err
	}
	...
	if !*detach {
		...
		go func() {
			errChan <- project.Log(true, cmd.Args()...)
		}()
		go func() {
			select {
			case <-signalChan:
				fmt.Printf("\nGracefully stopping...\n")
				project.Stop(0, cmd.Args()...)
				cleanupDone <- true
			case err := <-errChan:
				if err != nil {
					logrus.Fatal(err)
...
func (cli *DockerCli) CmdComposeStart(args ...string) error {
	...
	if err = project.Start(cmd.Args()...); err != nil {
		return err
	}
...
func (cli *DockerCli) CmdComposeStop(args ...string) error {
	...
	if err = project.Stop(*nSeconds, cmd.Args()...); err != nil {
		return err
	}
...
func (cli *DockerCli) CmdComposeCreate(args ...string) error {
	...
	err = project.Create(options, cmd.Args()...)
	if err != nil {
		return err
	}
...
func (cli *DockerCli) CmdComposeKill(args ...string) error {
	...
	err = project.Kill(*signal, cmd.Args()...)
	if err != nil {
		return err
	}
...
func (cli *DockerCli) CmdComposeRm(args ...string) error {
	...
	options := options.Delete{
		RemoveVolume: *removeVol,
	}
	err = project.Delete(options, cmd.Args()...)
	if err != nil {
		return err
	}
